...
class OptionContract(Contract):
    ...
    def from_str(s: str) -> OptionContract:
        try:
            count, strike, option_type, premium = s.split(" ")
            c = int(count)
            c_type = ContractType.LONG if c > 0 else ContractType.SHORT
            return OptionContract(abs(c), c_type, float(premium), OptionType[option_type.upper()], float(strike))
        except Exception as e:
            raise ValueError(f"Cant build an OptionContract object from input {s}. Error {e}")
...
class Position:
    ...
    def __init__(self, contracts: List[Contract]):
        ...
        for price in self.all_strikes:
            self.pl_at_strike[price] = self._total_pl_at_expiration(price)
    ...
    def _total_pl_at_expiration(self, exp_price: float):
        total_pl = 0
